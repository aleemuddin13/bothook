...
#if defined(NDEBUG)
# error "Bothook cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb5946c81e0e934894c779699224d9a5cdd1998ef1095adc81f38b4e1e23257d4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bothook: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bothook Signed Message:\n";
...
{
    // Bothook: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bothook: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Bothook:5 minutes
static const int64 nTargetSpacing = 1 * 60; // Bothook: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bothook: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bothook: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb5946c81e0e934894c779699224d9a5cdd1998ef1095adc81f38b4e1e23257d4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Bothook Created";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1505483740;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2518414;
        ...
        assert(block.hashMerkleRoot == uint256("0x0ede8a5a9cf901c341baa6168345f4ba0128ff54d23d9e9aef7cd946552b9406"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bothook: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BothookMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BothookMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BothookMiner(CWallet *pwallet)
{
    printf("BothookMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bothook-miner");
    ...
    try { loop {
        ...
        printf("Running BothookMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BothookMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BothookMiner, pwallet));
}
